...
def analyze_code(source_code: str, verbose: bool = True) -> dict:
    ...
    if parse_result.success:
        sem_result = run_semantic_analysis(source_code)
        results["semantic"] = sem_result.to_dict()
    else:
        results["semantic"] = None

    # ─── 5. CÓDIGO INTERMEDIO (TAC) ──────────────────────────────────────────
    if parse_result.success:
        ic_result = generate_intermediate_code(source_code)
        results["intermediate_code"] = ic_result.to_dict()
    else:
        results["intermediate_code"] = None
...
def _print_results(results: dict, source_code: str):
    ...
    ic_data = results.get("intermediate_code")
    if ic_data:
        print("=" * 60)
        print("⚙️  CÓDIGO INTERMEDIO (TAC):")
        print("=" * 60)
        instructions = ic_data.get("instructions", [])
        n_temps  = ic_data.get("temp_count", 0)
        n_labels = ic_data.get("label_count", 0)
        print(f"  Instrucciones: {len(instructions)}  |  Temporales: {n_temps}  |  Etiquetas: {n_labels}")
        print()

        # Reconstruir texto desde instrucciones para mostrar en terminal
        from src.intermediate_code_generator import TACInstruction
        for i, instr_dict in enumerate(instructions):
            instr = TACInstruction(
                op=instr_dict["op"],
                result=instr_dict.get("result"),
                arg1=instr_dict.get("arg1"),
                arg2=instr_dict.get("arg2"),
                label=instr_dict.get("label"),
                comment=instr_dict.get("comment") if instr_dict.get("op") == "comment" else None,
            )
            line = str(instr)
            if line.strip():
                if instr_dict["op"] in ("begin_func", "end_func"):
                    print(f"  {line}")
                else:
                    print(f"  {i+1:3d}  {line}")
        print()
